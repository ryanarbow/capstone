...
class ProfileExtractor(object):
    ...
    def data_for_profile(self, dv_ext):
        ...
        fee = soup.findAll(
            'span', {'class': 'dv-selected-service-rate__price'})[0].text\
            .strip()[1:]
        # Extract total number of reviews
        profile = soup.findAll('span', {'data-scroll-link': 'profile-reviews'})
        if profile is not None and len(profile) > 0:
            review = soup.findAll(
                'span', {'data-scroll-link': 'profile-reviews'})[0].text.\
        ...
        if response is not None and len(response) > 0:
            response_time = soup.findAll('li', {
                'class': 'dv-profile-list__item'})[1].text.replace(
                    'Responds within', '').strip()
        else:
            response_time = 0
        if 'minutes' in response_time:
            response_time = 1
        elif 'hour' in response_time:
            response_time = 2
        else:
            response_time = 3
        # Extract star rating
        stars = review_stars = soup.findAll(
            'ul', {'class': 'rating dv-review-stars'})
        if stars is not None and len(stars) > 0:
            review_stars = soup.findAll(
                'ul', {'class': 'rating dv-review-stars'})[0]
            full_star = review_stars.findAll(
                'i', {'class': ' dv-icon dv-icon__star '})
            rating = len(full_star)
            half_star = soup.findAll(
        ...
        # df1['response_time'] = df1['response_time'].map(lambda x: 1 if
        # 'minutes' in x else 2 if 'hour' in x else 3)
        # df1['review'] = df1['review'].map(lambda x: 0 if 'Testimonial' in x
        # else x)
    ...
    def data_for_cities(self):
        ...
        for src in os.listdir(self.data_dir):
            ...
            for sitter in sitters:
                rtwrap = sitter.findAll(
                    'i', {'class': 'dv-icon dv-icon__clock'})
                responder = sitter.findAll(
                    'li', {'class': 'dv-search-badge-item'})
                clock = len(rtwrap)
                if clock > 0:
                    response_time = responder[0].text.replace(
                        'Responds in', '').strip()
                else:
                    response_time = 0
                fee = sitter.findAll(
                    'span', {'class': "dv-host-list-item__price__amount"})[0]\
                    .text.replace('/night', '').strip()[1:]
                review = sitter.find('span', {'class': 'dv-host-review'})
                if review is not None and len(review) > 0:
                    review = review.text.replace(
                        'Reviews', '').replace('Review', '').strip()
                else:
                    review = 0
                full_star = sitter.findAll('i', {'class': 'dv-icon__star'})
                rating = len(full_star)
                half_star = sitter.findAll(
                    'i', {'class': 'dv-icon__star-half'})
                if len(half_star) > 0:
                    rating += .5
                town = soup.findAll(
                    'span', {'class': 'dv-hero__title__location'})[0].text\
                    .strip().split(',')[0]
                fees.append(fee)
        ...
        df2['response_time'] = df2['response_time'].map(
            lambda x: 1 if 'minutes' in x else 2 if 'hour' in x else 3)
        df2['review'] = df2['review'].map(
            lambda x: 0 if 'Testimonial' in x else x)
        ...
        df_2.rename(
            columns={
                'fee': 'fee_max',
                'rating': 'rating_max',
                'response_time': 'response_time_max',
                'review': 'review_max'}, inplace=True)
        df_3.rename(
            columns={
                'fee': 'fee_min',
                'rating': 'rating_min',
                'response_time': 'response_time_min',
                'review': 'review_min'}, inplace=True)
...
def test():
    dv_ext = DVExtractor()
    user_url = "https://dogvacay.com/best-care-in-the-west-end-dog-boarding-\
                242304?default_service=boarding"
