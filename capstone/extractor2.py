...
class ProfileExtractor(object):
    ...
    def data_for_profile(self, dv_ext):
        ...
        if profile is not None and len(profile) > 0:
            ...
            {'data-scroll-link': 'profile-reviews'})[0].text.replace('Reviews', 
        ...
        if stars is not None and len(stars) > 0:
            review_stars = soup.findAll('ul', {'class':
                'rating dv-review-stars'})[0]
            full_star = review_stars.findAll('i', {'class':
                ' dv-icon dv-icon__star '})
            rating = len(full_star)
            half_star = soup.findAll('i', {'class':
                ' dv-icon dv-icon__star-half '})
            if len(half_star) > 0:
                rating += .5
        else:
            rating = 0
        town = soup.findAll('div', {'class':
            'dv-profile-booking__address'})[0].text.strip().split(',')[0]
        state = soup.findAll('div', {'class':
            'dv-profile-booking__address'})[0].text.strip().split(',')[1]\
            .strip()[:2]
        #fees.append(fee)
    ...
    def data_for_cities(self):
        ...
        for src in os.listdir(self.data_dir):
            ...
            for sitter in sitters:
                rtwrap = sitter.findAll('i', {'class':
                    'dv-icon dv-icon__clock'})
                responder = sitter.findAll('li', {'class':
                    'dv-search-badge-item'})
                clock = len(rtwrap)
                if clock > 0:
                    response_time = responder[0].text.replace('Responds in',
                    '').strip()
                else:
                    response_time = 0
                fee = sitter.findAll('span', {'class':
                    "dv-host-list-item__price__amount"})[0].text.replace\
                    ('/night', '').strip()[1:]
                review = sitter.find('span', {'class': 'dv-host-review'})
                if review is not None and len(review) > 0:
                    review = review.text.replace('Reviews', '').replace\
                    ('Review', '').strip()
                else:
                    review = 0
                ...
                if len(half_star) > 0:
                    rating += .5
                town = soup.findAll('span', {'class':
                    'dv-hero__title__location'})[0].text.strip().split(',')[0]
                fees.append(fee)
                reviews.append(review)
                ratings.append(rating)
                city.append(town)
        ...
        'minutes' in x else 2 if 'hour' in x else 3)
        df2['review'] = df2['review'].map(lambda x: 0 if 'Testimonial' in x 
        else x)                   
